...
public:
	...
	void insert(int key, int val) {
		if (rootBid == 0) {
			...
			insert_leaf_node(key, val, tar, st);
		}
	}
	void insert_leaf_node(int key, int val, int Bid, stack<int> &st) {
		int* block = new int[blockSize / 4]{};
		bool full = true;
		...
		if (!full) {
			...
			for (i = 0;i <entry+1 - (entry + 1)/2;i++) {
				new_block2[2 * i] = tmp_block[2 * (i+ (entry + 1) / 2)];
				new_block2[2 * i + 1] = tmp_block[2 * (i+ (entry + 1) / 2) + 1];
			}
			...
			if (st.size() == 0) { // new_root
				...
				insert_non_leaf_node(new_block2[0], new_Bid, st);
			}
		}
	}
	void insert_non_leaf_node(int x, int chile_Bid, stack<int>& st) {
		int* block = new int[blockSize / 4]{};
		bool full = true;
		int Bid = st.top();
		st.pop();
		...
		if (!full) {
			...
			if (st.size() == 0) { // new_root
				...
				insert_non_leaf_node(K, new_Bid, st);
			}
		}
